...
const readText = async (relativePath: string): Promise<string> => {
  const file = Bun.file(
    new URL(`../../src/mainview/${relativePath}`, import.meta.url)
  );
...
const mainview = await readText("index.tsx");
const homeContent = await readText("home/home-content.tsx");
const homeScreen = await readText("home/home-screen.tsx");
const sidebar = await readText("sidebar/sidebar.tsx");
const playerBar = await readText("player/player-bar.tsx");

test("renderer bootstraps React from a minimal app root", () => {
  expect(html).toContain('<div id="app"></div>');
  expect(html).toContain('<link rel="stylesheet" href="index.css" />');
  expect(mainview).toContain("createRoot(appRoot).render(<App />)");
...
test("home cards preserve Pen geometry and hover affordances", () => {
  expect(css).toMatch(
    /\.home-hub-cards\s*\{[\s\S]*?grid-auto-columns: 150px;/u
  );
  expect(css).toMatch(
    /\.home-hub-card\s*\{[\s\S]*?width: 150px;[\s\S]*?gap: 10px;/u
  );
  expect(css).toMatch(
    /\.home-hub-card-text\s*\{[\s\S]*?width: 140px;[\s\S]*?gap: 2px;/u
  );
  expect(css).toMatch(
    /\.home-hub-card-title\s*\{[\s\S]*?font-size: 14px;[\s\S]*?line-height: 1\.25;/u
  );
  ...
  expect(css).toContain(
    ".home-hub-card-track:focus-within .home-hub-card-play"
  );
});

test("home card rows retain Pen scrolling and selector states", () => {
  expect(css).toMatch(
    /\.home-hub-cards\s*\{[\s\S]*?padding: 8px 3px 16px;[\s\S]*?margin: -8px -3px -8px;/u
  );
  expect(css).toMatch(
    /\.home-hub-cards\s*\{[\s\S]*?overflow-x: auto;[\s\S]*?scrollbar-width: none;/u
  );
