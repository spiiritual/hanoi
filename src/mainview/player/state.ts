...
export type PlaybackTrack = Pick<PlexTrack, "ratingKey" | "title"> &
  Partial<Pick<PlexTrack, "parentTitle" | "grandparentTitle" | "duration" | "thumb" | "index">>;
...
export function createPlayerState(options: PlayerOptions): PlayerState {
  ...
  const emitPlaybackMetric = (event: PlaybackMetric["event"], elapsedMs?: number) => {
  ...
  const markScrobbled = () => {
    ...
    if (previousAttempt?.status === "pending" || previousAttempt?.status === "succeeded") return;
  ...
  const maybeScrobble = () => {
    const duration = snapshot.duration || (snapshot.currentTrack?.duration ?? 0) / 1000;
  ...
  const removeSourceListeners = () => {
    if (audio) {
      for (const { type, listener } of sourceListeners) audio.removeEventListener?.(type, listener);
    }
    sourceListeners = [];
  };
  ...
  const addSourceListener = (source: SourceToken, type: string, handler: () => void) => {
    if (!audio) return;
  ...
  const installSourceListeners = (source: SourceToken, track: PlaybackTrack) => {
    ...
    addSourceListener(source, "play", () => setSnapshot({ status: "playing", error: null }));
    addSourceListener(source, "playing", () => {
      const elapsedMs = resumeStartedAt === null ? undefined : readNow() - resumeStartedAt;
  ...
  const loadTrack = async (index: number): Promise<void> => {
    ...
    try {
      ...
      if (!url) throw new Error("Plex could not find an audio file for this track.");
      ...
      if (generation !== loadGeneration) return;
      resumeStartedAt = null;
      if (!(cause instanceof Error && cause.name === "NotAllowedError")) sourceFailed = true;
      setSnapshot({
        status: "paused",
        error:
  ...
  return {
    ...
    togglePlay() {
      ...
      if (audio) {
        ...
        if (!audio.paused) {
          if (Number.isFinite(audio.currentTime)) setSnapshot({ currentTime: audio.currentTime });
          maybeScrobble();
          audio.pause();
    ...
    playNext() {
      ...
      return nextIndex < snapshot.queue.length ? loadTrack(nextIndex) : Promise.resolve();
    },
    playPrevious() {
      if (!snapshot.currentTrack) return Promise.resolve();
      ...
      return previousIndex >= 0 ? loadTrack(previousIndex) : loadTrack(snapshot.currentIndex);
    },
    seek(time) {
